...
class MigrationTest < Minitest::Test
  ...
  def test_it_generates_a_hash_with_renamed_attributes
    hash = { first_name: 'Homer', last_name: 'Simpson' }
    expected_hash = { firstname: 'Homer', lastname: 'Simpson' }
    config = Rex::Attribute.new(:config, nested: [ Rex::Attribute.new(:first_name, target: :firstname), Rex::Attribute.new(:last_name, target: :lastname) ])
    assert_equal(expected_hash, Rex::Migration.new.migrate(hash, config: config))
  end
end
